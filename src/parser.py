    ...
    try:
        ...
        if isinstance(e, KeyError):
            ...
            for error in e.errors():
                print(
                    f"-You are {error['type']} {error['loc']} {error['msg']}")
        exit(1)
...
def read_prompt(path: str) -> list[Prompt]:
    try:
        ...
        if not isinstance(data, list):
            raise ValueError(
                f"Please check your data syntax in this path {path}")
...
def build_prompt(all_functions, index, prompt):
    return (
        ...
        "even if they are whole numbers (e.g., "
        "2 -> 2.0). Never return integers."
        "Examples:\n"
